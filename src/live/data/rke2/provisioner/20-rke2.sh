...
sed -i "s|%VIPADDRESS%|$VAR_keepalivedVIPAddressKube|g" $rke2_config_dir/10-rke2-config-server-base.yaml-disabled
...
sed -i "s|%VIPADDRESS%|$VAR_keepalivedVIPAddressKube|g" $rke2_config_dir/10-rke2-config-server-address.yaml-disabled
